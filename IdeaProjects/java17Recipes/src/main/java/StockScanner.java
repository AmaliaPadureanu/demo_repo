...
public class StockScanner {
    ...
    public static void screen(List<String> stocks, Screen screen, double treshold) {
        Iterator<String> iter = stocks.iterator();
        while (iter.hasNext()) {
            String stock = iter.next();
            if (!screen(stock, screen, treshold)) {
                iter.remove();
            }
        }
    }
    ...
    public static void main (String[] args) {
        ...
        StockScanner.screen(stocks, Screen.YIELD, 3.5);
        System.out.println("Second screen: " + stocks);
        screen(stocks, Screen.PE, 22);
